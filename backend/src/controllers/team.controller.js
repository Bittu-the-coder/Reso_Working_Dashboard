...
const getTeam = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        if (!isMember) {
            return next(
                new ErrorResponse("You are not a member of this team", 403)
            );
        }
...
const deleteTeam = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        if (teamTasks.deletedCount > 0) {
            console.log(
                `Deleted ${teamTasks.deletedCount} tasks for team ${teamId}`
            );
        }
...
const addTeamMember = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        if (!isOwner && !isAdmin) {
            return next(
                new ErrorResponse(
                    "You are not authorized to add team members",
                    403
                )
            );
        }
        ...
        if (alreadyInvited || alreadyMember) {
            return next(
                new ErrorResponse("User is already invited or a member", 400)
            );
        }
        ...
        await notifyUserByEmail(
            userId,
            "Team Invitation",
            `You have been invited to join the team ${team.name} by ${req.user.fullName}. Please accept the invitation to become a member.`
        );
...
const getTeamMemberById = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        const member = team.members.find(
            (mem) => mem.userId?._id.toString() === userId
        );
        if (!member) {
...
const removeTeamMember = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        const isAdmin = team.members.some(
            (mem) =>
                mem.userId?._id.toString() === req.user._id.toString() &&
                mem.role === "admin"
        );
        ...
        if (!isAdmin) {
            return next(
                new ErrorResponse(
                    "You are not authorized to remove team members",
                    403
                )
            );
        }
        ...
        if (memberIndex === -1 && invitedMemberIndex === -1) {
            return next(
                new ErrorResponse("Member or invitation not found", 404)
            );
        }
...
const updateTeamMember = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        if (!isAdmin) {
            return next(
                new ErrorResponse(
                    "You are not authorized to update team members",
                    403
                )
            );
        }
        ...
        const targetUser = team.members.find(
            (mem) => mem._id.toString() === memberId
        );
        if (!targetUser) {
            return next(
